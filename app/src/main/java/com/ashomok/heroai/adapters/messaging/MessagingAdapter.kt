...
import com.ashomok.heroai.adapters.messaging.holders.HeaderHolder
import com.ashomok.heroai.adapters.messaging.holders.ReceivedTextHolder
import com.ashomok.heroai.adapters.messaging.holders.SentDeletedMessageHolder
import com.ashomok.heroai.adapters.messaging.holders.SentTextHolder
import com.ashomok.heroai.adapters.messaging.holders.TimestampHolder
...
class MessagingAdapter(
    ...
    override fun onBindViewHolder(mHolder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (type) {
            com.ashomok.heroai.model.constants.MessageType.SENT_TEXT -> {
            ...
            com.ashomok.heroai.model.constants.MessageType.RECEIVED_TEXT -> {
                val holder = mHolder as ReceivedTextHolder
                initHolder(holder)
                holder.bind(message, model)
            }

            com.ashomok.heroai.model.constants.MessageType.SENT_DELETED_MESSAGE -> {
    ...
    private fun distinctMessagesTimestamps() {
        ...
        for (i in messages.indices) {
            ...
            if (i == 0) {
                ...
                if (!com.ashomok.heroai.utils.TimeHelper.isSameDay(timestamp, oldTimestamp)) {
                    timestamps[i] = timestamp
                    lastTimestampPos = i
                }
            }
    ...
    fun messageInserted() {
        ...
        if (!com.ashomok.heroai.utils.TimeHelper.isSameDay(lastTimestamp, newTimestamp)) {
            timestamps[index] = newTimestamp
            lastTimestampPos = index
        }
    }
    ...
    private fun getHolderByType(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        when (viewType) {
            com.ashomok.heroai.model.constants.MessageType.DAY_ROW -> return TimestampHolder(
                LayoutInflater.from(parent.context).inflate(R.layout.row_day, parent, false)
            )

            com.ashomok.heroai.model.constants.MessageType.SENT_TEXT -> return SentTextHolder(
                context,
                LayoutInflater.from(parent.context)
                    .inflate(R.layout.row_sent_message_text, parent, false)
            )

            com.ashomok.heroai.model.constants.MessageType.RECEIVED_TEXT -> return ReceivedTextHolder(
                context,
                LayoutInflater.from(parent.context)
