package com.ashomok.chatoflegends.services
...
class NetworkService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (intent?.action != null) {
            val chatId = intent.getStringExtra(com.ashomok.chatoflegends.utils.IntentUtils.EXTRA_CHAT_ID)
            val messageId =
                intent.getStringExtra(com.ashomok.chatoflegends.utils.IntentUtils.EXTRA_MESSAGE_ID)
            if (intent.action == com.ashomok.chatoflegends.utils.IntentUtils.INTENT_ACTION_UPDATE_MESSAGE_STATE) {
                val state =
                    intent.getIntExtra(com.ashomok.chatoflegends.utils.IntentUtils.EXTRA_STAT, 0)
